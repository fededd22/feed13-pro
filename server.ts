...
function fetchServerLocation(): Promise<ServerLocationInfo | null> {
...
async function sendUserOwnConfig(chatId: number | string, userId: number | string, knownClient?: ClientConfig) {
  ...
  const isConnectedNow = !!(activeSockets[cli.id] && activeSockets[cli.id].size > 0);
...
async function sendConnectedDevicesReport(chatId: number | string) {
  const clients = getPersistedClients();
  const rows: { name: string; protocol: string; count: number; distinctIPs: number; isUserOwned: boolean }[] = [];
  ...
  for (const c of clients) {
    ...
    for (const s of Array.from(sockets)) {
      const ip = (s as any).clientIP;
      if (ip) ips.add(ip);
    }
    rows.push({ name: c.name, protocol: c.protocol, count, distinctIPs: ips.size || count, isUserOwned: !!c.isUserOwned });
  }

  rows.sort((a, b) => b.count - a.count);

  const totalDevices = rows.reduce((sum, r) => sum + r.count, 0);
  ...
  rows.forEach((r) => {
    ...
    text += `🔌 <b>${escapeHtml(r.name)}</b> (${r.protocol.toUpperCase()}) — ${r.count}${limitTag} جهاز`;
    if (r.distinctIPs !== r.count) text += ` (${r.distinctIPs} IP مختلف)`;
    text += `\n`;
  });
...
async function sendServerStatus(chatId: number | string) {
  ...
  for (const clientId of Object.keys(activeSockets)) {
    const count = activeSockets[clientId]?.size || 0;
    if (count > 0) {
...
async function sendClientsList(chatId: number | string, page: number = 1) {
  ...
  pageClients.forEach((c) => {
    ...
    const connectedNow = !!(activeSockets[c.id] && activeSockets[c.id].size > 0);
...
async function sendClientDetails(chatId: number | string, clientId: string) {
  ...
  const locationText = location ? `${location.flag} ${location.countryName}` : "🌍 غير معروف";
  const locationDetail = location ? formatServerLocationDetail(location) : "";
  const deviceSockets = activeSockets[cli.id];
  const deviceCount = deviceSockets ? deviceSockets.size : 0;
